...
- layout แนวตั้งสไตล์ในเกม (Dragon Saga sim): `computeLayout` จัด **row = depth** (สาย prerequisite ไหลบนลงล่าง) + **col = tidy-tree** (พ่อ center อยู่เหนือลูก, สายแตกกระจายเป็นคอลัมน์ข้างกัน) — เส้นเชื่อมลากจาก "ล่างไอคอนพ่อ → บนไอคอนลูก"
- กริด layout = ค่าคงที่ `NS/GX/GY/PAD` ที่หัว skill tree section ใน `app.js` (`NS`=ขนาดไอคอน, `GX`=ระยะห่างคอลัมน์, `GY`=ระยะห่างแถว) — **`GY` ต้อง ≥ ไอคอน + label 2 บรรทัด (~81px)** ไม่งั้นชื่อสกิลถูกแถวล่างทับ/ตัด (ปัจจุบัน `GY=94`)
